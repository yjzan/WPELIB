...
var EditorModule = yjzanModules.Module.extend({

	onInit: function onInit() {
		jQuery(window).on('yjzan:init', this.onYjzanReady);
	},
	...
	onYjzanReady: function onYjzanReady() {
		this.onYjzanInit();

		yjzan.on('frontend:init', this.onYjzanFrontendInit.bind(this)).on('preview:loaded', this.onYjzanPreviewLoaded.bind(this));
	}
});

EditorModule.prototype.onYjzanInit = function () {};

EditorModule.prototype.onYjzanPreviewLoaded = function () {};

EditorModule.prototype.onYjzanFrontendInit = function () {};
...
var _class = function (_yjzanModules$Mod) {
	...
	_createClass(_class, [{
		...
		value: function initDialog() {
			...
			this.getDialog = function () {
				if (!dialog) {
                    ...
                    dialog = yjzanCommon.dialogsManager.createWidget(settings.dialogType, settings.dialogOptions);
		...
		value: function setViewed() {
			...
			yjzanCommon.ajax.addRequest('introduction_viewed', {
				data: {
...
ControlsStack = Marionette.CompositeView.extend({
	classes: {
		popover: 'yjzan-controls-popover'
	},
	...
	className: function className() {
		return 'yjzan-controls-stack';
	},

	templateHelpers: function templateHelpers() {
		return {
			elementData: yjzan.getElementData(this.model)
		};
	},
	...
	ui: function ui() {
		return {
			...
			reloadButton: '.yjzan-update-preview-button'
		};
	},
	...
	initCollection: function initCollection() {
		this.collection = new Backbone.Collection(_.values(yjzan.mergeControlsSettings(this.getOption('controls'))));
	},
	...
	activateTab: function activateTab(tabName) {
		...
		this.ui.tabs.removeClass('yjzan-active').filter('[data-tab="' + tabName + '"]').addClass('yjzan-active');
	...
	getNamespaceArray: function getNamespaceArray() {
        return [yjzan.getPanelView().getCurrentPageName()];
	},
	...
	openActiveSection: function openActiveSection() {
		...
		if (activeSectionView[0]) {
			activeSectionView[0].$el.addClass('yjzan-open');

			var eventNamespace = this.getNamespaceArray();

            eventNamespace.push(activeSection, 'activated');

			yjzan.channels.editor.trigger(eventNamespace.join(':'), this);
		}
	},
	...
	onDeviceModeChange: function onDeviceModeChange() {
		if ('desktop' === yjzan.channels.deviceMode.request('currentMode')) {
			this.$el.removeClass('yjzan-responsive-switchers-open');
		}
	},

	onChildviewControlSectionClicked: function onChildviewControlSectionClicked(childView) {
		var isSectionOpen = childView.$el.hasClass('yjzan-open');
	...
	onChildviewResponsiveSwitcherClick: function onChildviewResponsiveSwitcherClick(childView, device) {
		if ('desktop' === device) {
			this.$el.toggleClass('yjzan-responsive-switchers-open');
		}
	}
});
...
BaseSettingsModel = Backbone.Model.extend({
	...
	initialize: function initialize(data, options) {
		...
		self.controls = yjzan.mergeControlsSettings(options.controls);
		...
		_.each(self.controls, function (control) {
			...
			if (undefined !== attrs[controlName] && isMultipleControl && !_.isObject(attrs[controlName]) && !hasDynamicSettings) {
				yjzan.debug.addCustomError(new TypeError('An invalid argument supplied as multiple control value'), 'InvalidElementData', 'Element `' + (self.get('widgetType') || self.get('elType')) + '` got <' + attrs[controlName] + '> as `' + controlName + '` value. Expected array or object.');
				...
				if (!(attrs[field.name] instanceof Backbone.Collection)) {
					attrs[field.name] = new Backbone.Collection(attrs[field.name], {
						model: function model(attributes, options) {
							...
							if (!attributes._id) {
								attributes._id = yjzan.helpers.getUniqueID();
							}
	...
	getStyleControls: function getStyleControls(controls, attributes) {
		...
		controls = yjzanCommon.helpers.cloneObject(self.getActiveControls(controls, attributes));

		var styleControls = [];

		jQuery.each(controls, function () {
			var control = this,
			    controlDefaultSettings = yjzan.config.controls[control.type];
	...
	getActiveControls: function getActiveControls(controls, attributes) {
		...
		_.each(controls, function (control, controlKey) {
			if (yjzan.helpers.isActiveControl(control, attributes)) {
	...
	clone: function clone() {
		return new BaseSettingsModel(yjzanCommon.helpers.cloneObject(this.attributes), yjzanCommon.helpers.cloneObject(this.options));
	},
	...
	parseDynamicSettings: function parseDynamicSettings(settings, options, controls) {
		...
		settings = yjzanCommon.helpers.cloneObject(settings || self.attributes);
		...
		jQuery.each(controls, function () {
			...
			if (undefined === dynamicSettings) {
				dynamicSettings = yjzan.config.controls[control.type].dynamic;
			}
			...
			try {
				dynamicValue = yjzan.dynamicTags.parseTagsText(valueToParse, dynamicSettings, yjzan.dynamicTags.getTagDataContent);
			} catch (error) {
				if (yjzan.dynamicTags.CACHE_KEY_NOT_FOUND_ERROR !== error.message) {
					throw error;
				}
				...
				yjzan.dynamicTags.refreshCacheFromServer(function () {
	...
	toJSON: function toJSON(options) {
		...
		return yjzanCommon.helpers.cloneObject(data);
	}
});
