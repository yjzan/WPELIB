...
jQuery(function () {
	yjzanProAdmin.assetsManager.fontManager.init();
	yjzanProAdmin.roleManager.advancedRoleManager.init();
});
...
module.exports = function () {
	...
	self.init = function () {
		jQuery(document).on('change', '.yjzan-widget-template-select', function () {
			var $this = jQuery(this),
			    templateID = $this.val(),
			    $editButton = $this.parents('p').find('.yjzan-edit-template'),
			    type = $this.find('[value="' + templateID + '"]').data('type');

			if ('page' !== type) {
				// 'widget' is editable only from Yjzan page
				$editButton.hide();

				return;
			}

			var editUrl = YjzanProConfig.i18n.home_url + '?p=' + templateID + '&yjzan';
...
module.exports = function (key, fieldID) {
	...
	self.cacheElements = function () {
		this.cache = {
			$button: jQuery('#yjzan_pro_' + key + '_button'),
			$apiKeyField: jQuery('#yjzan_pro_' + key),
			$apiUrlField: jQuery('#yjzan_pro_' + fieldID)
		};
	};
...
module.exports = function () {
	...
	self.selectors = {
		editPageClass: 'post-type-yjzan_font',
		title: '#title',
		...
		customFontsMetaBox: '#yjzan-font-custommetabox',
		closeHandle: 'button.handlediv',
		toolbar: '.yjzan-field-toolbar',
		inlinePreview: '.inline-preview',
		fileUrlInput: '.yjzan-field-file input[type="text"]'
	};
	...
	self.updateRowLabel = function (event, $table) {
		...
		return self.renderTemplate(self.fontLabelTemplate, {
			...
			preview: '<span style="' + previewStyle.styleAttribute + '">Yjzan is making the web beautiful</span>',
			toolbar: toolbarHtml
		});
	};
...
module.exports = {
	...
	selectors: {
		uploadBtnClass: 'yjzan-upload-btn',
		clearBtnClass: 'yjzan-upload-clear-btn',
		uploadBtn: '.yjzan-upload-btn',
		clearBtn: '.yjzan-upload-clear-btn'
	},
	...
	setUploadParams: function setUploadParams(ext, name) {
		...
		self.fileFrame[name].uploader.uploader.param('uploadeTypecaller', 'yjzan-admin-upload');
	},
...
module.exports = {
	selectors: {
		...
		repeaterField: '.yjzan-field-repeater'
	},
...
module.exports = function () {
	...
	self.cacheElements = function () {
		this.cache = {
			$button: jQuery('#yjzan_pro_typekit_validate_button'),
			$kitIdField: jQuery('#yjzan_typekit-kit-id'),
			$dataLabelSpan: jQuery('.yjzan-pro-typekit-data')
		};
	};
	self.bindEvents = function () {
		this.cache.$button.on('click', function (event) {
	...
	self.fetchFonts = function () {
		...
		jQuery.post(ajaxurl, {
			action: 'yjzan_pro_admin_fetch_fonts',
			kit_id: kitID,
			_nonce: self.cache.$button.data('nonce')
		}).done(function (data) {
			if (data.success) {
...
module.exports = function () {
	...
	self.cacheElements = function () {
		this.cache = {
			$checkBox: jQuery('input[name="yjzan_exclude_user_roles[]"]'),
			$advanced: jQuery('#yjzan_advanced_role_manager')
		};
	};
	self.bindEvents = function () {
	...
	self.init = function () {
		if (!jQuery('body').hasClass('yjzan_page_yjzan-role-manager')) {
			return;
		}
		this.cacheElements();
		this.bindEvents();
...
module.exports = function () {
	var selectors = {
		templateTypeInput: '#yjzan-new-template__form__template-type',
		locationWrapper: '#yjzan-new-template__form__location__wrapper',
		postTypeWrapper: '#yjzan-new-template__form__post-type__wrapper'
	};
	...
	var setElements = function setElements() {
		jQuery.each(selectors, function (key, selector) {
			key = '$' + key;
			elements[key] = yjzanNewTemplate.layout.getModal().getElements('content').find(selector);
		});
	};
	...
	this.init = function () {
		if (!window.yjzanNewTemplate) {
			return;
		}

		// Make sure the modal has already been initialized
		yjzanNewTemplate.layout.getModal();
